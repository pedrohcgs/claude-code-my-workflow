...
description: Mechanize Pattern 15 — the seven-pass adversarial review protocol for academic manuscripts. Spawns 7 parallel subagents (abstract, intro, methods, results, robustness, prose, citations), then synthesizes a prioritized revision checklist. Use for submission-ready or R&R-stage papers where single-pass review isn't enough.
...
**Why seven passes?** A single-agent review blends lenses and softens each one. Seven parallel agents each approach the paper with full context budget for their own lens, then a synthesizer resolves conflicts and de-duplicates.
...
Each lens runs as an independent subagent so the main conversation stays clean.
...
In a single message, spawn 7 subagents (one per lens). Each subagent gets:
...
Lens prompt rubrics are embedded inline below — one summary paragraph per lens. Each subagent receives its lens's rubric plus the manuscript path.
