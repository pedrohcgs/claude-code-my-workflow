...
Before returning the ideation report, run the Post-Flight Verification protocol from [`.claude/rules/post-flight-verification.md`](../../../.claude/rules/post-flight-verification.md). Research ideation is hallucination-prone in three specific ways:
...
3. **Spawn the `claim-verifier` subagent** with claims, questions, and source pointers (WebSearch allowed, NBER/SSRN URLs preferred, dataset codebooks preferred). Do NOT include the draft.
