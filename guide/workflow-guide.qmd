...
## Corrections Log
- [LEARN:r-code] Always use cbind(1, X) for DRDID low-level functions
...
In Econ 730, we found that DRDID low-level functions silently produce **catastrophically biased estimates** without an explicit intercept column (`cbind(1, X)`). This bug was caught 3 times in different scripts. Without the replication-first protocol, these wrong numbers would have been taught to PhD students.
:::
...
- [LEARN:citation] Post-LASSO is Belloni (2013), NOT Belloni (2014)
- [LEARN:r-code] DRDID low-level: ALWAYS cbind(1, X)
- [LEARN:workflow] Every Beamer edit must auto-sync to Quarto
```
