...
Skills are multi-step workflows invoked with `/command`.

In this repo, there are two skill layers:

- **Core academic workflow skills** live in `.claude/skills/[name]/SKILL.md`
- **Project-local Codex skills** live in `.agents/skills/[name]/SKILL.md`
- When a project-local skill should also appear in Claude-style discovery, it can be bridged back into `.claude/skills/` as a symlink

The core template skills follow this structure:
...
Phase 2: CHECK EXISTING
  Search .claude/skills/ and .agents/skills/ for related skills
  → Nothing related? Create new. Overlap? Update existing.
         ↓
Phase 3: CREATE SKILL
  Write to the matching layer:
  • .claude/skills/[name]/SKILL.md for core slash-command workflow
  • .agents/skills/[name]/SKILL.md for project-local Codex extensions
  • Problem statement
...
Skills committed under `.claude/skills/` or `.agents/skills/` survive compaction and session boundaries --- if you discover something valuable late in a session, extract it with `/learn` before compaction erases the details.
...
The **kill switch** is explicit: at any point, you can stop, archive with a one-paragraph explanation, and move on. No guilt, no sunk cost. Work that stays entirely inside `explorations/` uses the fast-track rule as the explicit exception to the repo's general plan-first workflow. See `.claude/rules/exploration-folder-protocol.md` and `.claude/rules/exploration-fast-track.md` for the full protocols.
...
Core template skills are directories in `.claude/skills/` with a `SKILL.md` file. Project-local Codex skills follow the same structure under `.agents/skills/`.

The file format is the same in both places:
...
1. **Choose the skill layer:**
   - Use `.claude/skills/` for core reusable slash-command workflow
   - Use `.agents/skills/` for project-local Codex extensions that should travel with the repo

2. **Copy the template:**
   ```bash
   mkdir -p .agents/skills/your-skill-name
   cp templates/skill-template.md .agents/skills/your-skill-name/SKILL.md
   ```

   If you want the same project-local skill visible in `.claude/skills/`, add a bridge symlink:
   ```bash
...
This table lists the **core academic workflow skills** that live in `.claude/skills/`. Project-local Codex skills in `.agents/skills/` are intentionally repo-specific and may be bridged into `.claude/skills/` when shared discovery is useful.
