...
```
.claude/rules/
├── plan-first-workflow.md         # ~100 lines — plan before you build
...
├── pipeline-isolation.md          # ~50 lines — critics never create
└── inter-agent-communication.md   # ~30 lines — mailbox protocol
```
...
::: {.callout-warning}
## Effort Regression Fix

If Claude seems to skip research and jump straight to editing, the default `medium` effort may not be enough for complex multi-file work. Three fixes:

1. `/effort high` (or `/effort max` on Opus for hard debugging)
2. Add `"showThinkingSummaries": true` to `settings.json` --- this lets you verify Claude is actually thinking before acting
3. Add "Research the codebase before editing. Never change code you haven't read." to your `CLAUDE.md` Core Principles

The template already includes `showThinkingSummaries: true` in `settings.json` and the research-before-editing principle in `CLAUDE.md`. Skills like `/data-analysis` use `effort: max` in their frontmatter to ensure thorough analysis.
:::
...
Patterns 1--8 apply broadly, with course materials as the primary example. Patterns 9--12 are designed for **research projects** --- papers, simulations, and empirical analysis --- where the rhythm is different: ideas are uncertain, experiments may fail, and code is often written to answer a question rather than to ship. Patterns 13--15 extend the foundation to reproducibility standards, presentation rhetoric, and multi-audit protocols. Patterns 16--19 integrate **external tools and advanced automation**: second-opinion reasoning (Oracle), PDF preprocessing (LiteParse), autonomous experiment loops (autoresearch), and pipeline isolation for review integrity.
...
## Advanced Patterns {#sec-beyond-slides}

The patterns above use slides and research as primary examples, but the infrastructure is domain-agnostic. The following patterns address reproducibility, presentation design, multi-audit protocols, external tool integration, and pipeline quality assurance.
...
Before feeding papers to `/lit-review`, `/review-paper`, or `/oracle`, preprocess them with [LiteParse](https://github.com/run-llama/liteparse) --- a local-first PDF parser that runs entirely on your machine (no data leaves your computer).

**Install:** `npm i -g @llamaindex/liteparse` (requires Node.js) or `brew install llamaindex-liteparse`

```
/parse-paper master_supporting_docs/angrist_pischke_2009.pdf
```

**Text extraction** works well for prose, abstracts, and references from modern digital PDFs. **Screenshots** (generated via `lit screenshot paper.pdf --dpi 300`) are better for tables, figures, and equations --- text extraction mangles mathematical notation. For a full literature review, batch-process your reading list:

```bash
lit batch-parse ./reading_list/ ./parsed_texts/ --recursive --extension .pdf
```

The parsed output goes to `/tmp/` by default (exploratory, not committed). Move to `master_supporting_docs/parsed/` if you want to keep it.
...
The template includes 28 [skills](#skills---reusable-slash-commands) for common academic tasks. But if you have repetitive workflows specific to your domain, you can create your own.
...
| Rule | File | Purpose |
|------|------|---------|
| Plan-First Workflow | `plan-first-workflow.md` | Plan mode + deep comprehension + context preservation |
...
| Pipeline Isolation | `pipeline-isolation.md` | Critics never create, creators never self-score |
| Inter-Agent Communication | `inter-agent-communication.md` | Mailbox protocol for agent coordination |
...
| Foreground agents | PreToolUse[Agent] (command) | `.claude/hooks/enforce-foreground-agents.sh` |
| Plan persistence | Stop (command) | `.claude/hooks/plan-reminder.py` |
