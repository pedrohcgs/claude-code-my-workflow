* ============================================================================
* 4_summary_stats.do --- summary statistics table + histograms
* ============================================================================
* Source: project/data/function/code/ff_summary_stats.do
* Project: Fertilizer Quality in Kenya
*
* Does:
*   - generates summary statistics table (.tex)
*   - produces histograms of nitrogen content by fertilizer type and round
*   - produces histogram of nitrogen content by manufacturer
*
* Assumes:
*   - config.do has been run (sets $output, $figures globals)
*   - estout package installed (see 0_setup.do)
*   - 3_store_distance.do has produced mystery_addvars.dta

cap: log close
log using "$output/ff_summary_stats", replace

use "$output/mystery_addvars.dta", clear
...
graph export "$figures/purchases.png", replace
...
esttab using "$figures/summary_stats.tex", cells("count(fmt(a2)) mean(fmt(a2)) sd(fmt(a2)) min(fmt(a2)) max(fmt(a2))") noobs replace nonumber starlevels(* .1 ** .05 *** .01) label title("Summary statistics")

hist percent if numberround == 1 & round == 1, by(bag_type) scheme(plotplain) freq
graph export "$figures/hist_quality_by_type.png", replace
...
graph export "$figures/hist_dap.png", replace
...
graph export "$figures/hist_can.png", replace
...
graph export "$figures/hist_urea.png", replace
...
graph export "$figures/hist_can_manuf.png", replace
