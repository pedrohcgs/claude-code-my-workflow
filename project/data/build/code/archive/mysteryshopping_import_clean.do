...
* log using "$dir/build/output/mysteryshopping_import_clean", replace

*Import mystery shopping data*
import delimited "$dir\build/input\MysteryShoppingr2.csv", clear
...
gen num=_n
save "$dir\build/output\MysteryShopping_r1.dta", replace
restore
...
save "$dir\build/output\MysteryShopping_r2.dta", replace

* Import label ids for the mystery shopped bags from r1
import delimited "$dir\build/input\label_ids.csv", clear
gen round = 1
merge 1:1 num round using "$dir\build/output\MysteryShopping_r1.dta"
drop _merge
...
save "$dir\build/output\MysteryShopping_r1V2.dta", replace

* Add in test results
* Open results and save as .dta
import delimited "$dir\build/input\MIR predicted total CN reported.csv", clear
* rename merging variable
save "$dir\build/input/mir_predicted_total_CN_reported.dta", replace

use "$dir/build/input/raw_mir_spectra.dta", clear
keep ssn - treatment

merge 1:1 ssn using "$dir/build/input/mir_predicted_tool_total_CN_reported.dta", nogen
* everything merges
...
merge 1:1 label_id using "$dir\build/output\MysteryShopping_r1V2.dta"
save "$dir\build/output\MysteryShopping_r1V3.dta", replace

* Import label ids for the mystery shopped bags from r2

import delimited "$dir\build/input\label_idsr2.csv", clear
merge 1:1 id bag_no	round using "$dir\build/output\MysteryShopping_r2.dta"
drop _merge
drop num
save "$dir\build/output\MysteryShopping_r2V2.dta", replace

* Add in test results

import delimited "$dir\build/input\Migori fertilizer cn_pxrf data.csv", clear
rename ssn label_id
destring total_nitrogen, replace
merge 1:1 label_id using "$dir\build/output\MysteryShopping_r2V2.dta"
save "$dir\build/output\MysteryShopping_r2V3.dta", replace

append using "$dir\build/output\MysteryShopping_r1V3.dta"
drop _merge
...
*Missing Information* //this adds information about the sealed bags if they were the '4th sample'--the survey only allowed characteristics to be recorded
...
replace bag_issues1=0 if id==147 & bag_no==4 & round ==1  //hard time telling if it's too dark because of lighting
...
label var ferttype_flag "Wrongly Coded"
	* the lab thought these were wrongly coded
*/
...
gen percent = diff/expected_nitrogen
gen flag = 1 ==(!mi(comment))
lab var flag "Lab thought it might be mis-labeled"

drop X*

save "$dir\build/output\MysteryShopping_all.dta", replace
