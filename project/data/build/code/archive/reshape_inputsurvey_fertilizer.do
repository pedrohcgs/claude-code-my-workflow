...
import delimited "$dir\InputSurvey-fert_questions.csv", rowrange(13:) clear
...
merge 1:m parent_key using "$dir\build\output\InputSurvey-fert_questions_r1.dta"
drop _merge
...
/*might want to drop ID 11 from data? The store just opened and as of survey time only sold Foliar Feed. The owner was planning to bring in mineral fertilizer, but didn't have any for interview, so all questions pertain to foliar feed (this was before we updated survey to exclude foliar feed from study--will need to drop from all relevant .do files
drop if id==11 */
...
save "$dir\build/output\manufacturer_fertilizer.dta", replace
