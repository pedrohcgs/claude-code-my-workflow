...
* Note: file directory is set in "general set up" below and all files in this do-file are in sub-folders of that directory, and are referred to based on that path.

* *does
	* imports fertilizer survey data (Homabay & Migori)
	* some data cleaning + create some new variables
...
* local user "et2"
if "`user'"=="et2" {
	global dropbox "C:\Users\etjernstrom\Dropbox" 
}
	
	global dir "$dropbox\Fake fertilizer\PEDL\data"

*********** general setup
cd "$dir"
cap: log close
set scheme plotplain
log using "$dir/build/code/agrovet_survey", replace
***********
...
import delimited "$dir/build/input/InputSurvey.csv", rowrange(5:) clear

*rename parent_key in order to merge datasets*
rename key parent_key

label data "Agrodealer survey \ $S_DATE"
note: InputSurvey.dta \ agrodealer survey w/ basic cleaning \ created using agrovet_survey.do \ etj \ $S_DATE
save "$dir/build/output/InputSurvey.dta", replace
...
import delimited "$dir/build/input/InputSurvey-fert_questions.csv", rowrange(13:) clear
...
save "$dir/build/output/InputSurvey-fert_questions_r1.dta", replace

merge m:1 parent_key using "$dir\build\output\InputSurvey.dta", nogen
* All but 2 obs merge
...
* replace purchase_unit_large = 2 if price_small > 600 & purchase_unit == 5 & (fert_type_id == 1 | fert_type_id == 2)
* For some reason some obs are straight up missing purchase_unit_large -- easy to infer what size when the prices are really large:
...
* hist fert_number_total if number ==1, scheme(plottig) freq
...
replace cost=. if cost==0 //change all 0 costs to missing
replace price=. if price==0 //an old version of the survey asked specifically for the price of 50kg bags--
//enumerators entered 0 if the store didn't sell 50 kg bags. Survey was updated after first few days to allow for smaller bag sizes. 

**might want to drop ID 11 from data? The store just opened and as of survey time only sold Foliar Feed. The owner was planning to bring in 
//mineral fertilizer, but didn't have any for interview, so all questions pertain to foliar feed (this was before we updated survey to 
//exclude foliar feed from study--will need to drop from all relevant .do files
drop if id==11 

**drop store 58?--owner indicated they didn't plan to sell fertilizer again in the future**
* drop if id==58
...
* Some obvious mismatches between sale unit, sale price, and cost (noticed when defining markup) so need a different variable
...
save "$dir/build/output/agrovet_survey_cleanish.dta", replace


**********************
* Make a data version with just store IDs, lat/long for distance calculations & maps
...
note: agrovet_survey_gps.dta \ agrodealer survey w/ basic cleaning \ created using agrovet_survey.do \ etj \ $S_DATE
save "$dir/build/output/agrovet_survey_gps.dta", replace
