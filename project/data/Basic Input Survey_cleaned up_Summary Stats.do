...
import delimited "InputSurvey.csv", rowrange(5:) clear 
...
forvalues i = 1(1)189 {
	local id = id[`i']
	di "local i =" `i' " and local id = " `id' " and gpslongitude[`id'] = " gpslongitude[`id']
	gen lon`id' = gpslongitude[`id']
}

forvalues i = 1(1)189 {
	local id = id[`i']
	cap: gen lat`id' = gpslatitude[`id']
}	
...
forvalues i = 1(1)189 {
	...
	/* attempting to check for the distances to see if it looks right
...
forvalues i = 1(1)2 {
	...
	forval j = 1(1)189 {
		local id2 = id[`j']
		cap: geodist lon`id' lat`id' lon`id2' lat`id2', gen(dist`id'_`id2')
		cap: gen lessthan5_`id'_`id2' = 1 if dist`id'_`id2' <=5
		}
	egen tempvar = rowtotal(dist`id'_`id2')
	cap: replace within_5 = tempvar-1 if id == `id'
//	cap: drop dist`id'_`id2' tempvar
	}
