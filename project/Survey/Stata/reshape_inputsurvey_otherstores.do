...
*Import Supplier Questions CSV--excludes Vihiga data*
import delimited "$dir\data\InputSurvey-other_stores", rowrange(7:) clear


save "InputSurvey_other_stores.dta", replace
...
merge 1:m parent_key using "InputSurvey_other_stores.dta", nogen
sort id key
...
**Labels for other_stores data**
label define distance_unit 0 "meters" 1 "km"
label values distance_unit distance_unit

label values other_store_county county

order other_store_name-key, after(setofother_stores)
...
save "inputsurvey_otherstores.dta", replace

**Merge Network IDs**
import delimited "$dir\data\network_IDs", clear
gen n_id=_n
order n_id, first

keep n_id id other_store_id

merge 1:1 n_id id using "inputsurvey_otherstores.dta", nogen

drop n_id
order other_store_id, before(other_store_name)

*Incorporate enumerator comments*
replace other_store_phone =. if other_store_phone==0 //unknown store phone numbers
...
save "inputsurvey_otherstores_networkid.dta", replace
