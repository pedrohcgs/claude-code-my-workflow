...
*Import Supplier Questions CSV*
import delimited "$dir\data\InputSurvey-supplier_questions.csv", clear


/*
**Generate ID to match with InputSurvey data**
gen id = 1 if supplier_nr==1
gen storeid = sum(id)
drop id
rename storeid id
order id, first
...
save "InputSurvey_supplier_storeid.dta", replace
...
*Merge Input Survey with Supplier Questions*
merge 1:m parent_key using "InputSurvey_supplier_storeid.dta", nogen
sort id supplier_nr
...
**Label Supplier_Questions Data**
label define supplier 0 "Retailer" 1 "Wholesaler" 2 "Manufacturer" 99 "Other, specify"
label values supplier_type supplier

label values supplier_credit yesno
label values repayment_interest yesno

label define repayment 0 "Days" 1 "Weeks" 2 "Months"
label values repayment_term_unit repayment

split fertilizer_type, destring
	forval i =1/`r(nvars)'{
		label variable fertilizer_type`i' "Fertilizer Type #`i'"
		}
	order fertilizer_type1-fertilizer_type4, after(fertilizer_type)
	drop fertilizer_type
	label values fertilizer_type1-fertilizer_type4 fert_type

split supply_month, destring
	forval i =1/`r(nvars)'{
		label variable supply_month`i' "Supply Month #`i'"
		}
	order supply_month1-supply_month12, after(supply_month)
	drop supply_month
	label values supply_month1-supply_month12 months
	
order supplier_nr-key, after(setofsupplier_questions)
...
replace supplier_loc="Doesn't know" if id==35 & supplier_loc=="Nairobi"
replace supplier_loc="Doesn't know" if id==134 & supplier_nr==1
replace supplier_loc="Doesn't know" if id==43 & supplier_nr==1
replace supplier_loc="Doesn't know" if id==110 & supplier_nr==2 | id==110 & supplier_nr==3
replace supplier_loc="Doesn't know" if id==93 & supplier_nr==1

replace supplier_type=2 if id==111 & supplier_nr==1
replace supplier_loc="" if id==111 & supplier_nr==1

replace supplier_phone=. if supplier_phone==0
...
save "inputsurvey_supplierquestions.dta", replace
