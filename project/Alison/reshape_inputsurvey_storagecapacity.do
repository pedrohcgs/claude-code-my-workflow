...
*Imports storage capacity CSV file--excludes Vihiga data*
import delimited "$dir\data\InputSurvey-storage_capacity.csv", rowrange(5:) clear
...
save "InputSurvey-storage_capacity_wide.dta", replace

*Import Input Survey csv--excludes Vihiga data*
import delimited "$dir\data\InputSurvey.csv", rowrange(5:) clear

merge 1:1 key using "InputSurvey-storage_capacity_wide.dta", nogen
sort id
...
label define business_type 0 "Permanent" 1 "Seasonal" 2 "Mobile" 3 "Other, specify"
...
label values storage_unit__* units
...
split runout_cope, destring
	...
	drop runout_cope
label define runout 0 "Purchase more from regular supplier" 1 " Top up from local dealer (not regular)" 2 "Direct customers to another agrovet" 3 " Purchase more from nearby town" 4 "Nothing" 99 "Other, specify"
...
**reshape long**
reshape long storage_nr_calc__ storage_capacity__ storage_unit__ facility_characteristic__ setofstorage_capacity__, i(id) j(storageunit)	
drop if missing(storage_nr_calc__)

**split and destring storage facility characteristics**
split facility_characteristic__, destring
	forval i =1/`r(nvars)'{
		label variable facility_characteristic__`i' "Facility Characteristic #`i'"
		}
	order facility_characteristic__1-facility_characteristic__4, after(facility_characteristic__)
	drop facility_characteristic__
	
label define facility 0 "Tin roof" 1 "Concrete floor" 2 "Inventory stored on pallets" 3 "Inventory stored on shelves" 4 "Fan ventilation" 5 "Climate controlled (air conditioning)"
label values facility_characteristic__1-facility_characteristic__4 facility

*reorder storage capacity data*
order storage_nr_calc__-setofstorage_capacity__, after(setofstorage_capacity)
drop setofstorage_capacity__
 ...
 **None of the quantities of stored fertilizer are populating for the 2nd and 3rd storage units--need to figure out why**
 
replace storage3=2 if id==87
replace storageo=" " if id==87
...
*Incorporate enumerator comments*
replace storage_capacity__=0 if id==30 
...
save "inputsurvey_storagecapacity", replace
